  ...
  if(ISDEBUG){
    ...
    if( callerParts.length >= 1 ){
      ...
      if(typeof process !== "undefined"){// node js
        ...
        }else if(stack.indexOf(" file:///") !== -1){// array object?
          //console.log(stack)
          console.log(chalk.green('Console log:'), stack.slice(stack.indexOf(" file:///"),stack.indexOf("at process")).trim())
        }else if(stack.indexOf("webpack-internal:///(api)/./lib/log") !== -1){
          //console.log(stack)
          console.log(chalk.green('Console log:'), stack.slice(stack.indexOf("at _callee$ (webpack-internal:"),stack.indexOf("at tryCatch")).trim())
        }else{
          console.log(chalk.green('Console log:'), path.join(cwdOSRoot,callerParts[2])+":"+ callerParts[3] );
        }
        ...
        }else if(stack.match("at dispatch")){
          console.log(stack.slice(0,stack.indexOf("at dispatch")).trim())
        }else if(stack.match("at tryCatch")){
          console.log(stack.slice(0,stack.indexOf("at tryCatch")).trim())
        }else{
